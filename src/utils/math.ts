import { IColumnsByPin, ITableColumn } from '@/types/column';
import { columnsByPin, columnsTotalWidth } from './column';

/**
 * Calculates the Total Flex Grow
 */
function getTotalFlexGrow(columns: ITableColumn[]) {
  let totalFlexGrow = 0;
 ...
 */
export function adjustColumnWidths(allColumns: ITableColumn[] = [], expectedWidth: number) {
  // if (allColumns && allColumns.length) {
  //   allColumns = allColumns.filter(c => c.visible && !c.hidden);
  // }
  const hiddenColumns: ITableColumn[] = [];
 ...
 */
function scaleColumns(colsByGroup: IColumnsByPin, maxWidth: number, totalFlexGrow: number) {
  // calculate total width and flexgrow points for coulumns that can be resized
  let column: ITableColumn;
  for (const attr in colsByGroup) {
    for (column of colsByGroup[attr as keyof IColumnsByPin]) {
  ...
  const hasMinWidth: Record<string, any> = {};
  let remainingWidth = maxWidth;

  // resize columns until no width is left to be distributed
  do {
    const widthPerFlexPoint = Math.floor(remainingWidth / totalFlexGrow);
    remainingWidth = 0;
    for (const attr in colsByGroup) {
      for (column of colsByGroup[attr as keyof IColumnsByPin]) {
...
function calcRealWidth(column: ITableColumn): number {
  if (!column.element) {
    return null;
  }
...
export function forceFillColumnWidths(
  ...
  const averageColumnWidth = Math.ceil(expectedWidth / columnsToResize.length);
  ...
  const columnsProcessed: ITableColumn[] = [];

  // This loop takes care of the
  do {
    additionWidthPerColumn = Math.ceil(remainingWidth / columnsToResize.length);
 ...
 */
function removeProcessedColumns(columnsToResize: ITableColumn[], columnsProcessed: ITableColumn[]) {
 ...
 */
function getContentWidth(allColumns: ITableColumn[], defaultColWidth: number = 300): number {
  let contentWidth = 0;
